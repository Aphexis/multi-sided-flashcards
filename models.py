...
class Cell_SQL(db.Model):
    ...
    card_id = db.Column(db.ForeignKey('cards.card_ID'), nullable=False, index=True)
    side_id = db.Column(db.ForeignKey('sides.side_id'), nullable=False, index=True)
    info = db.Column(db.String(1000))

    card = db.relationship('Card_SQL')
    side = db.relationship('Side_SQL')
